...
% set parameters
% for lambdaW = [1 1.1 0.9]
% par.lambdaW = lambdaW;

for nSig = 0.2:0.01:0.25
    par.nSig = nSig;
    for lambda = [0.1:0.1:0.5 0]
        par.lambda = lambda;
        PSNR = [];
        SSIM = [];
        CCPSNR = [];
        ...
        mCCPSNR = mean(CCPSNR);
        mCCSSIM = mean(CCSSIM);
        save(['WLS_In_Sigma_1AG_RID_' num2str(lambda) '_' num2str(nSig) '.mat'],'alltime','mtime','PSNR','mPSNR','SSIM','mSSIM','CCPSNR','mCCPSNR','CCSSIM','mCCSSIM');
    end
end
% end