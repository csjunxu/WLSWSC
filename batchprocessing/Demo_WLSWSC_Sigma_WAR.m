...
par.ps = 6;        % patch size
par.win = 30;   % size of window around the patch
...
for lambdasc = [0.01 0.05 0.1]
    ...
    for lambdals = 1:-0.1:0.7
        ...
        for i = 1 : im_num
            ...
            par.nim = IMin;
            par.imIndex = i;
            par.nSig = NoiseEstimation(par.nim, 8);
            t1=clock;
        ...
        mCCPSNR = mean(CCPSNR);
        mCCSSIM = mean(CCSSIM);
        save(['WLSWSC_Sigma_WAR_WWIter' num2str(par.WWIter)  '_lls' num2str(lambdals) '_lsc' num2str(lambdasc) '.mat'],'alltime','mtime','PSNR','mPSNR','SSIM','mSSIM','CCPSNR','mCCPSNR','CCSSIM','mCCSSIM');
    end
end