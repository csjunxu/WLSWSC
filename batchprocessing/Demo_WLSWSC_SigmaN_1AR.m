...
for nSig0 = 0.1:0.05:0.25
    par.nSig0 = nSig0;
    for lambdals = 1:.1:1.5
        par.lambdals = lambdals;
        for lambdasc = 3.1:0.1:3.2
            ...
            for i = 1 : im_num
                ...
                par.imIndex = i;
                t1=clock;
                [IMout, par]  =  WLSWSC_SigmaN_1AR(par);
                t2=clock;
                etime(t2,t1)
            ...
            mCCPSNR = mean(CCPSNR);
            mCCSSIM = mean(CCSSIM);
            save(['WLSWSC_SigmaN_1AR_nSig0' num2str(nSig0) '_lls' num2str(lambdals) '_lsc' num2str(lambdasc) '_WWIter' num2str(par.WWIter) '.mat'],'alltime','mtime','PSNR','mPSNR','SSIM','mSSIM','CCPSNR','mCCPSNR','CCSSIM','mCCSSIM');
        end
    end
end